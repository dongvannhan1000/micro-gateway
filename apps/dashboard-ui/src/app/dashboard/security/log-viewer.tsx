...
export function SecurityLogViewer({ initialLogs, projects, initialProjectId }: SecurityLogViewerProps) {
    ...
    const filteredLogs = logs.filter(log => 
        ...
        (log.prompt_injection_score >= 0.7 ? 'blocked' : 'suspicious').includes(searchQuery.toLowerCase())
    );
    ...
    return (
        <div className="space-y-8">
            ...
            <div className={clsx("glass overflow-hidden border-glass-border transition-opacity", loading ? "opacity-30 pointer-events-none" : "opacity-100")}>
                    ...
                    <tbody className="divide-y divide-glass-border">
                        {filteredLogs.length > 0 ? filteredLogs.map((log) => (
                            <tr key={log.id} className="hover:bg-glass-bg/30 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <code className="text-[10px] bg-glass-bg px-2 py-1 rounded text-accent-blue italic font-mono">{log.model}</code>
                                </td>
                                <td className="px-6 py-4">
